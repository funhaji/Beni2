...
import { restoreFromBackup, type BackupData } from "../lib/backup.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const result = await restoreFromBackup(body as BackupData);
    ...
    res.status(500).json({
      ok: false,
      error: "Restore failed: " + String((error as Error).message || error)
    });
  }
}
