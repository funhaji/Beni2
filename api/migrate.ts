...
function formatExpiry(expireMs: number): string {
  if (!expireMs || expireMs <= 0) return "unlimited";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (action === "lookup") {
      ...
      if (!subLink) return res.status(400).json({ ok: false, error: "subLink is required" });
    ...
    if (action === "provision") {
      ...
      if (!subLink) return res.status(400).json({ ok: false, error: "subLink is required" });
      if (!targetPanelId) return res.status(400).json({ ok: false, error: "targetPanelId is required" });
      ...
      if (isPasarGuard) {
        ...
        const groups = await getPasarguardGroups(
          String(targetPanel.base_url),
          login.token
        );
        if (groups.length === 0) {
        ...
        const inboundsRes = await fetchWithTimeout(
          `${normalizeBaseUrl(String(targetPanel.base_url))}/api/inbounds`,
          { headers: { Authorization: `Bearer ${login.token}`, Accept: "application/json" } }
        );
        const inboundsRaw = await inboundsRes.text();
        const inboundsData = parseJsonObject(inboundsRaw) as Record<string, unknown[]> | null;

        const inboundsMap: Record<string, string[]> = {};
        const proxiesMap: Record<string, Record<string, never>> = {};
        if (inboundsData && typeof inboundsData === "object") {
          for (const [proto, entries] of Object.entries(inboundsData)) {
            if (!Array.isArray(entries) || entries.length === 0) continue;
            const tags = entries
              .map((e) => String((e as Record<string, unknown>).tag || ""))
              .filter(Boolean);
            if (tags.length > 0) {
      ...
      const createRes = await fetchWithTimeout(
        `${normalizeBaseUrl(String(targetPanel.base_url))}/api/user`,
        {
          method: "POST",
          headers: {
      ...
      const createData = parseJsonObject(createRaw) as Record<string, unknown> | null;
      ...
      const links = Array.isArray(createData.links)
        ? (createData.links as unknown[]).map((l) => String(l || "")).filter(Boolean)
        : [];
