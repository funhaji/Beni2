...
import { exportAllTables } from "../lib/backup.js";
...
async function fetchTable(table: ValidTable): Promise<Record<string, unknown>[]> {
  switch (table) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (tableParam) {
      if (!(VALID_TABLES as readonly string[]).includes(tableParam)) {
        res.status(400).json({ ok: false, error: "Unknown table: " + tableParam });
        return;
      }
      const rows = await fetchTable(tableParam as ValidTable);
