...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === "scan_marzban") {
      ...
      if (!listRes.ok) throw new Error("Failed to fetch Marzban users");
      const listData = parseJsonObject(await listRes.text()) as any;
      const users = Array.isArray(listData?.users) ? listData.users : [];
      const total = Number(listData?.total || 0);

      const deadUsers = users.filter((u: any) => {
        return u.status === "limited" || u.status === "expired";
      });

      const deadUsernames = deadUsers.map((u: any) => String(u.username));
      ...
      const mappedDead = deadUsers.map((u: any) => {
    ...
    if (action === "scan_sanaei") {
      ...
      for (const inbound of inbounds.items) {
        ...
        const settings = parseJsonObject(String(inbound.settings || "{}")) as any;
        ...
        for (const client of clients) {
          const stat = clientStats.find((s: any) => s.email === client.email);
          if (!stat) continue;
      ...
      if (identifiers.length > 0) {
        ...
        inventoryMatches.forEach((row) => {
          ...
          identifiers.forEach((id) => {
            if (val.includes(id)) dbMap.set(id, Number(row.id));
          });
        });
      }
