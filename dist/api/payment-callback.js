...
// ─── Plisio ───────────────────────────────────────────────────────────────────
function normalizePlisioPaymentId(data) {
...
function isPlisioPaid(status) {
    const s = status.toLowerCase().trim();
    return s === "completed" || s === "mismatch";
}
function isPlisioFailure(status) {
    const s = status.toLowerCase().trim();
    return s === "expired" || s === "cancelled" || s === "error" || s === "cancelled duplicate";
}
async function handlePlisio(req, res) {
...
// ─── Tronado ──────────────────────────────────────────────────────────────────
async function handleTronado(req, res) {
    ...
    try {
        ...
        await notifyAdmins(`⚠️ خطا در تایید پرداخت Tronado\nسفارش: ${paymentId}\nعلت: ${statusErr.message || String(statusErr)}`);
...
// ─── SwapWallet ───────────────────────────────────────────────────────────────
async function handleSwapwallet(req, res) {
    const { getSwapwalletInvoiceById, getSwapwalletInvoiceByOrderId, isSwapwalletInvoicePaid } = await import("../lib/swapwallet.js");
    ...
    try {
        ...
        await notifyAdmins(`⚠️ خطا در بررسی پرداخت SwapWallet\norderId: ${orderId || "-"}\ninvoiceId: ${invoiceId || "-"}\nعلت: ${e.message || String(e)}`);
...
// ─── Tetrapay ─────────────────────────────────────────────────────────────────
async function handleTetrapay(req, res) {
    ...
    try {
        ...
        await notifyAdmins(`⚠️ خطا در تایید پرداخت TetraPay\nauthority: ${authority}\nعلت: ${statusErr.message || String(statusErr)}`);
