...
let cachedSocksAgent;
let cachedHttpProxyAgent;
function proxySocksUrl() {
    const url = env.PROXY_SOCKS_URL?.trim();
    if (!env.PROXY_ENABLED || !url)
        return undefined;
    return url;
}
function buildSocksAgent() {
...
export function shouldUseProxyForUrl(url) {
    if (!isProxyEnabled())
        return false;
...
function telegramApiBases() {
    const bases = [];
    const primary = env.TELEGRAM_API_BASE?.trim();
    if (primary)
        bases.push(primary.replace(/\/$/, ""));
 ...
 */
export async function fetchWithProxyFallback(url, init = {}, options) {
...
const QR_PROVIDERS = [
    (text) => `https://quickchart.io/qr?size=320&margin=2&text=${encodeURIComponent(text)}`,
    (text) => `https://api.qrserver.com/v1/create-qr-code/?size=320x320&data=${encodeURIComponent(text)}`
];
/** Download QR image (proxied). Used so Telegram does not fetch blocked QR URLs itself. */
export async function fetchQrImageBuffer(text) {
