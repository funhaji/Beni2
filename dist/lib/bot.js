...
function parseExpiryInputToDays(input) {
    ...
    if (str === "یک ماه" || str === "یکماه")
        return 30;
    if (str === "دو ماه" || str === "دوماه")
        return 60;
    if (str === "سه ماه" || str === "سهماه")
        return 90;
    if (str === "شش ماه" || str === "ششماه")
        return 180;
    if (str === "یک هفته" || str === "یک‌هفته" || str === "یکهفته")
        return 7;
    if (str === "دو هفته" || str === "دوهفته")
        return 14;
    if (str === "یک سال" || str === "یکسال")
        return 365;
...
function formatExpiryDaysLabel(days) {
    ...
    if (days === 7)
        return "۱ هفته (۷ روز)";
    if (days === 14)
        return "۲ هفته (۱۴ روز)";
    if (days < 30)
        return `${days} روز (هفتگی)`;
    if (days === 30)
        return "۱ ماهه (۳۰ روز)";
    if (days > 30 && days < 60)
        return `${days} روز (~۱ ماهه)`;
    if (days === 60)
        return "۲ ماهه (۶۰ روز)";
    if (days > 60 && days < 90)
        return `${days} روز (~۲ ماهه)`;
    if (days === 90)
        return "۳ ماهه (۹۰ روز)";
    if (days === 180)
        return "۶ ماهه (۱۸۰ روز)";
    if (days === 365)
        return "۱ ساله (۳۶۵ روز)";
    const months = Math.round(days / 30);
    return `${months} ماهه (${days} روز)`;
}
function getProductExpiryDays(p) {
    const cfg = sanitizePanelConfig(p.panel_config);
    return parseMaybeNumber(cfg.expire_days || cfg.days) ?? 30;
}
function getProductExpiryCategory(days) {
    if (days < 30) {
...
async function promptProductPanelWizardStep(chatId, payload) {
    ...
    if (step === "expire_days") {
        ...
        await tg("sendMessage", {
            chat_id: chatId,
            text: `${isQuick ? "⚡ تنظیم سریع فروش پنل" : "تنظیم مرحله‌ای - 5 از 5"}\n` +
...
async function promptProductWizardStep(chatId, payload) {
    ...
    if (step === "expire_days") {
        ...
        await tg("sendMessage", {
            chat_id: chatId,
            text: `محصول ${mode === "add" ? "جدید" : "ویرایش"}\n` +
                `مدت زمان انقضا/اعتبار کانفیگ را مشخص کنید:\n` +
                `می‌توانید یکی از دکمه‌ها را بزنید یا عدد/متن بفرستید.\n` +
                `نمونه‌ها: 30 یا 1 ماه یا 7 روز یا 2 ماه\n` +
                (mode === "edit" ? `\nمقدار فعلی: ${formatExpiryDaysLabel(curr)}` : "") +
...
async function showProducts(chatId, forBuy, page = 0, kind = "", expiryCategory = "") {
    const globalInfinite = await getBoolSetting("global_infinite_mode", false);
    ...
    `;
        ...
        if ((v2rayCount > 0 ? 1 : 0) + (accountCount > 0 ? 1 : 0) + (wireguardCount > 0 ? 1 : 0) > 1) {
            ...
            if (accountCount > 0)
                keyboard.push([cb("👤 اشتراک", "buy_cat_account_0", "primary")]);
            if (wireguardCount > 0)
                keyboard.push([cb("🛡 وایرگارد (Wireguard)", "buy_cat_wireguard_0", "primary")]);
            keyboard.push([homeButton()]);
  ...
  `;
    ...
    if (forBuy && kind === "v2ray") {
        if (expiryCategory) {
            displayRows = standardRows.filter((p) => {
                const days = getProductExpiryDays(p);
...
async function listProductsForAdmin(chatId, userId, page = 0, kindFilter) {
  ...
  `;
    ...
    if (totalPages > 1) {
        const navRow = [];
        if (safePage > 0)
            navRow.push({ text: "◀️ قبلی", callback_data: `admin_products_${kindSuffix}_page_${safePage - 1}` });
        navRow.push({ text: `صفحه ${safePage + 1} از ${totalPages}`, callback_data: "noop" });
        if (safePage < totalPages - 1)
            navRow.push({ text: "بعدی ▶️", callback_data: `admin_products_${kindSuffix}_page_${safePage + 1}` });
        keyboard.push(navRow);
    }
    ...
    if (kindFilter === "v2ray")
        title = "مدیریت کانفیگ‌ها (V2Ray):";
    if (kindFilter === "account")
        title = "مدیریت اشتراک‌ها (VPN):";
    if (kindFilter === "wireguard")
        title = "مدیریت وایرگارد (Wireguard):";
    await tg("sendMessage", {
        chat_id: chatId,
        text: title,
        reply_markup: { inline_keyboard: keyboard }
...
async function handleCallback(update) {
    ...
    if (data.startsWith("admin_product_set_expiry_")) {
        ...
        await tg("sendMessage", {
            chat_id: chatId,
            text: `⏱ تنظیم مدت زمان اعتبار (انقضا) برای «${rows[0].name}»\n\n` +
    ...
    if (data === "admin_product_wizard_infinite_yes" || data === "admin_product_wizard_infinite_no") {
        ...
        const payload = { ...state.payload, isInfinite, step: "expire_days" };
        await setState(userId, "admin_product_wizard", payload);
        await promptProductWizardStep(chatId, payload);
        return null;
    }
    ...
    if (data.startsWith("admin_product_wizard_delivery_")) {
        ...
        const payload = { ...state.payload, panelDeliveryMode, step: "expire_days" };
        await setState(userId, "admin_product_wizard", payload);
        await promptProductWizardStep(chatId, payload);
        return null;
    }
    ...
    if (data === "admin_product_panel_quick") {
        ...
        const payload = { ...state.payload, quickMode: true, step: "expire_days" };
        await setState(userId, "admin_product_panel_wizard", payload);
        await promptProductPanelWizardStep(chatId, payload);
        return null;
    }
    if (data.startsWith("admin_product_panel_expire_")) {
        ...
        const payload = { ...state.payload, expireDays: days, step: "data_limit_mb" };
        await setState(userId, "admin_product_panel_wizard", payload);
        await promptProductPanelWizardStep(chatId, payload);
        return null;
    }
