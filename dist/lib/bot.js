...
async function parseAndApplyState(chatId, userId, text, photoFileId, state) {
    ...
    if (state.state === "admin_config_builder_wizard") {
      ...
      `;
            ...
            await clearState(userId);
            try {
                await sendDeliveryPackage(targetUserId, purchaseId, provision.configValue, { ...delivery, metadata }, [[{ text: "🏠 منوی اصلی", callback_data: "home" }]], "🎁 یک کانفیگ جدید برای شما صادر شد.");
            }
            catch (error) {
    ...
    `;
        ...
        await clearState(userId);
        await sendDeliveryPackage(Number(order.telegram_id), String(order.purchase_id), String(text), { configLinks: [String(text)] }, [[{ text: "🏠 منوی اصلی", callback_data: "home" }]]);
...
async function applyAdminResetUsageOnMarzban(panel, username) {
    const login = await loginMarzbanPanel({
        base_url: String(panel.base_url),
        username: String(panel.username || ""),
        password: String(panel.password || "")
...
async function applyAdminResetUsageOnSanaei(panel, inboundId, email) {
    const login = await loginSanaeiPanel({
        base_url: String(panel.base_url),
        username: String(panel.username || ""),
        password: String(panel.password || "")
...
export async function applyAdminSetExpiryOnMarzban(panel, username, expiryTimeMs) {
    ...
    const payload = { ...getData, expire: expiryTimeMs > 0 ? Math.floor(expiryTimeMs / 1000) : 0 };
    const putRes = await fetchWithTimeout(`${baseUrl}/api/user/${encodeURIComponent(username)}`, {
        method: "PUT",
        headers: {
            Authorization: `Bearer ${login.token}`,
...
async function createOrder(chatId, userId, productId, paymentMethod, discountInput, walletUsedParam = 0) {
    const globalInfinite = await getBoolSetting("global_infinite_mode", false);
    const rows = await sql `
    SELECT
      p.id,
  ...
  `;
    ...
    const { discountAmount, discountCode } = await resolveDiscount(discountInput, Number(product.price_toman));
    let walletUsed = 0;
    let finalPrice = Math.max(1, Number(product.price_toman) - discountAmount);
    ...
    `;
        ...
        await sql `
      INSERT INTO wallet_transactions (telegram_id, amount, type, description, created_at)
      VALUES (${userId}, ${negativeWalletUsed}, 'purchase', ${`خرید محصول ${product.name} (سفارش ${purchaseId})`}, NOW());
    `;
    ...
    if (data.startsWith("check_order_")) {
        ...
        try {
            ...
            if (paymentMethod === "tetrapay") {
                ...
                const result = await getStatusByPaymentId(purchaseId);
