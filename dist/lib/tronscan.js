...
export async function getRecentTrxTransfers(address, limit = 50) {
    ...
    if (!res.ok)
        throw new Error(`tronscan_http_${res.status}`);
    ...
    for (const row of rows) {
        ...
        if (!txid || !to || !Number.isFinite(amount) || amount <= 0 || timestampMs <= 0)
            continue;
