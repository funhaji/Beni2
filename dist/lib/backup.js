...
export async function exportAllTables() {
    ...
    for (const table of BACKUP_TABLES) {
        try {
            ...
            switch (table) {
                case "settings":
                    tables[table] = await sql `SELECT * FROM settings;`;
                    break;
                case "users":
                    tables[table] = await sql `SELECT * FROM users;`;
                    break;
                case "panels":
                    tables[table] = await sql `SELECT * FROM panels;`;
                    break;
                case "products":
                    tables[table] = await sql `SELECT * FROM products;`;
                    break;
                case "inventory":
                    tables[table] = await sql `SELECT * FROM inventory;`;
                    break;
                case "discounts":
                    tables[table] = await sql `SELECT * FROM discounts;`;
                    break;
                case "cards":
                    tables[table] = await sql `SELECT * FROM cards;`;
                    break;
                case "payment_methods":
                    tables[table] = await sql `SELECT * FROM payment_methods;`;
                    break;
                case "crypto_wallets":
                    tables[table] = await sql `SELECT * FROM crypto_wallets;`;
                    break;
                case "banned_users":
                    tables[table] = await sql `SELECT * FROM banned_users;`;
                    break;
                case "orders":
                    tables[table] = await sql `SELECT * FROM orders;`;
                    break;
                case "wallet_transactions":
                    tables[table] = await sql `SELECT * FROM wallet_transactions;`;
                    break;
                case "wallet_topups":
                    tables[table] = await sql `SELECT * FROM wallet_topups;`;
                    break;
                case "referral_rewards":
                    tables[table] = await sql `SELECT * FROM referral_rewards;`;
                    break;
                case "topup_requests":
                    tables[table] = await sql `SELECT * FROM topup_requests;`;
                    break;
                case "panel_migrations":
                    tables[table] = await sql `SELECT * FROM panel_migrations;`;
                    break;
                case "config_forensics":
                    tables[table] = await sql `SELECT * FROM config_forensics;`;
                    break;
                default:
                    tables[table] = [];
                    break;
...
export async function restoreFromBackup(data) {
  ...
  `;
    ...
    for (const table of ORDER) {
        restored[table] = await ins(table, (t[table] ?? []));
    }
