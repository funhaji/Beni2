...
type ProductKind = "v2ray" | "account" | "wireguard";
type ProductWizardStep = "pingchi_plan_id"
  | "price_toman_wireguard"
  | "name"
  | "product_kind"
  | "size_mb"
  | "price_mode"
...
async function promptProductWizardStep(chatId: number, payload: Record<string, unknown>) {
  ...
  if (step === "name") {
    await tg("sendMessage", {
      chat_id: chatId,
      text:
        `محصول ${mode === "add" ? "جدید" : "ویرایش"}${mode === "edit" && productKind === "wireguard" ? " (وایرگارد)" : " - 1 از 9"}\n` +
  ...
  if (step === ("price_toman_wireguard" as any)) {
    await tg("sendMessage", {
      chat_id: chatId,
      text:
        `ویرایش محصول وایرگارد\n` +
...
async function promptCardWizardStep(chatId: number, payload: Record<string, unknown>) {
  ...
  if (step === "card_number") {
    await tg("sendMessage", {
      chat_id: chatId,
      text:
        `کارت ${mode === "add" ? "جدید" : "ویرایش"} - 2 از 4\nشماره کارت را بفرستید.` +
...
async function parseAndApplyState(
  ...
  if (state.state === "admin_product_wizard") {
    ...
    if (step === "name") {
      ...
      return true;
    } else if (step === ("price_toman_wireguard" as any)) {
      ...
      if (result.ok) await listProductsForAdmin(chatId, userId, 0, "wireguard");
      return true;
...
async function handleCallback(update: TgUpdate["callback_query"]) {
  ...
  if (data === "admin_product_wizard_price_auto") {
    ...
    const payload = { ...state.payload, priceMode: state.payload.productKind === "account" || state.payload.productKind === "wireguard" ? "manual" : "auto", step: "sell_mode" as ProductWizardStep };
    await setState(userId, "admin_product_wizard", payload);
    await promptProductWizardStep(chatId, payload);
    return null;
  }
